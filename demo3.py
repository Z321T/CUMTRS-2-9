...
class UserBasedCFEcommerce:
    def __init__(self, k_neighbors=50, shrinkage=20, min_common=2,
                 ...
                 min_item_support=5, sim_batch_size=1000):
        """
        基于用户协同过滤的电商推荐系统(内存优化版)
    ...
    def compute_user_similarity(self):
        ...
        print("  [2/4] 中心化评分矩阵...")
        # 稀疏矩阵中心化: R_centered[i,j] = (R[i,j] - mean[i]) if R[i,j] > 0
        centered_data = []
        centered_rows = []
        centered_cols = []
        ...
        for batch_idx in tqdm(range(n_batches), desc="  批次进度"):
            ...
            # 提取当前批次的中心化矩阵
            batch_centered = centered_iuf[start_u:end_u]
            batch_norms = norms[start_u:end_u]

            # 计算余弦相似度: (batch @ all^T) / (||batch|| * ||all||)
            sim_batch = (batch_centered @ centered_iuf.T).toarray()  # 只转换结果
            ...
            # 强制垃圾回收
            del sim_batch, common_batch, batch_centered, batch_binary
            gc.collect()
        ...
        del centered, centered_iuf, binary_matrix, iuf_diag
        gc.collect()

    def predict_user_items(self, user_idx, top_n=30):
        """预测单个用户的商品评分(基于TopK邻居)"""
...
def main():
    ...
    rec_sys = UserBasedCFEcommerce(
        ...
        max_rows=50_000_000,
        min_item_support=5,
        sim_batch_size=1000  # 减小批次避免内存峰值
    )
